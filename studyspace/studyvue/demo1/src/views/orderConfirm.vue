...
<!--            <button class="btn btn&#45;&#45;m" @click="previous()">Previous</button>-->
            <router-link class="btn btn--m" to="cartdress">Previous</router-link>
          </div>
    ...
    export default {
        ...
        methods:{
            init(){
              ...
              this.cartList = JSON.parse(data).data
              this.cartList.forEach((item)=>{
                if (item.checked == 1){
                  this.subTotal += parseInt(item.productNum) * parseFloat(item.productPrice)
                }
              })
            ...
            payment(){
              ...
              console.log(addressId,this.orderTotal)
              this.$router.push({
                path: '/orderSuccess?orderId='+ new Date().getTime() + '&orderTotal=' + this.orderTotal
              })
            }
        }

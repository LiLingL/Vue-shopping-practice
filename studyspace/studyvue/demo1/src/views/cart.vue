<template>
  <div>
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartData.data" :key="item.productId">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a class="checkbox-btn item-check-btn" :class="{'check':item.checked}" @click.stop="checkChange(item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click.stop="changeNum('sub',item)">-</a>
                        <span class="select-ipt">{{ item.productNum}}</span>
                        <a class="input-add" @click.stop="changeNum('add',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a @click="deleteData(item.productId)" href="javascript:;" class="item-edit-btn">
          ...
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: <span class="total-price">{{togglePrice | currency('￥') }}</span>
              </div>
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':checkCount == 0}" @click.stop="checkOut()">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <modal :cardShow="deleteShow" @cardClose="cardClose()">
      <p slot="message" class="loginCont-p">确定删除该物品吗？</p>
      <div slot="mess-btn">
...
<script>
  ...
  // import {currency} from './../util/currecncy'
      ...
      checkAllFlag:{
        get(){
          if (this.cartData.data){
            if (this.checkCount == this.cartData.data.length) {
              return  1
            }else {
              return 0
            }
    ...
    methods: {
      ...
      deleteSure(){
        for (let i = 0; i < this.cartData.data.length; i++) {
          if (this.cartData.data[i].productId == this.proId){
            this.cartData.data.splice(i,1)
          }
        }
        window.localStorage.setItem("cartData", JSON.stringify(this.cartData))
        this.deleteShow = false

      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressListFilter" :key="item.addressId" :class="{'check': checkIndex == index}" @click="selectItem(index,item.addressId)">
                  <dl>
                    <dt>{{ item.userName }}</dt>
                  ...
                  <div class="addr-opration addr-del">
                    <a class="addr-del-btn" @click.stop="delAddressConfirm(item.addressId)">
                      <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                    </a>
                  </div>
                  <div class="addr-opration addr-set-default">
                    <a class="addr-set-default-btn" v-if="!item.isDefault" @click.stop="setDefault(item.addressId)"><i>Set default</i></a>
                  </div>
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" @click.stop="expand()" :class="{'open': limit>3}">
                more
                <i class="i-up-down">
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'addressId':selectId}}">next</router-link>
          </div>
        </div>
      </div>
    </div>
    <modal :cardShow="deleteShow" @cardClose="cardClose()">
      <p slot="message" class="loginCont-p">您确定删除该条地址吗？</p>
      <div slot="mess-btn">
        <button @click.stop="cardClose()" class="loginCont-pbtn">取消</button>
        <button @click.stop="deleteSure()" class="loginCont-pbtn">确定</button>
      </div>
...
<script>
    ...
    export default {
      ...
      methods:{
          init(){
            var data = window.localStorage.getItem("addressInfo")
            ...
            if (aa) {
              ...
              axios.get('/api/seller').then((response)=>{
                if(response.status == 200) {
                  this.addressList = response.data.data.result;
                  window.localStorage.setItem("addressInfo", JSON.stringify(this.addressList))
                } else {
                  this.addressList = []
                }
              })
          ...
          deleteSure () {
            for (let i = 0; i < this.addressList.length; i++) {
              if (this.addressList[i].addressId == this.delId){
